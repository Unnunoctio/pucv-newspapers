...
class RadioUChile:
    def __init__(self) -> None:
        self.SITE_NAME = "RADIO_UCHILE"
        self.BASE_URL = "https://radio.uchile.cl/noticias/"
    ...
    def get_total_pages(self) -> int:
        body = self.fetcher.fetch_page(f"{self.BASE_URL}page/1/")
        if body is None:
            return 0
        
        soup = BeautifulSoup(body, "html.parser")

        page_items = soup.select(".pagination li a.page-numbers")
        last_page_url = page_items[-2].get("href")
        return int(last_page_url.split("/")[-2])
    ...
    def get_start_page(self, start_date: datetime, start_page: int, end_page: int) -> int:
        ...
        soup = BeautifulSoup(body, "html.parser")
        page_items = soup.select(".post")
        # DATE (dd-mm-yyyy)
        last_paper_date_str = page_items[-1].select("ul.meta li")[-1].get_text(strip=True)
    ...
    async def async_get_paper_urls(self, session: aiohttp.ClientSession, page: str) -> list[str]:
        ...
        links = soup.select(".post .details .post-title a")
        urls = set()
        for elem in links:
    ...
    async def async_get_paper(self, session: aiohttp.ClientSession, url: str) -> Paper | None:
        ...
        try:
            ...
            paper.set_radio_uchile_data(body)
            return paper
